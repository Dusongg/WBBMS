...
v2 已接入限流与熔断，提升服务稳定性与防雪崩能力。

### 7.1 限流

| 能力 | 说明 |
|------|------|
| 算法 | Token Bucket（golang.org/x/time/rate） |
|  scope | 全局或按 IP（`resilience.rate-limit.per-ip`） |
| 默认 | 100 RPS，burst 50，按 IP |
| 跳过路径 | /healthz、/readyz、/metrics、/api/healthz、/api/readyz |
| 超限响应 | HTTP 429 |

配置：`config.yaml` → `resilience.rate-limit`；环境变量 `RATE_LIMIT_*`。

### 7.2 熔断

| 能力 | 说明 |
|------|------|
| 组件 | gobreaker（DB / Redis 独立熔断器） |
| 驱动 | /readyz 的 DB Ping、Redis Ping 调用 |
| Redis | 通过 Hook 自动包裹所有 Redis 命令 |
| 熔断打开时 | 非健康检查请求直接返回 503 |

配置：`config.yaml` → `resilience.circuit-breaker`；环境变量 `CIRCUIT_BREAKER_*`。
...
# 熔断：停 Redis → 连续失败 → 熔断打开 → 503；重启 Redis 后 timeout 进入半开恢复
```
...
6. **限流与熔断**：Token Bucket 限流防瞬时打垮；gobreaker 熔断防止 Redis/DB 故障级联
