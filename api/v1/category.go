...
func (c *CategoryApi) GetCategoryList(ctx *gin.Context) {
	...
	if err := global.DB(ctx.Request.Context()).Order("sort ASC, name ASC").Find(&categories).Error; err != nil {
...
func (c *CategoryApi) CreateCategory(ctx *gin.Context) {
	...
	if err := global.DB(ctx.Request.Context()).Unscoped().Where("name = ?", category.Name).First(&existCategory).Error; err == nil {
		// 如果存在软删除的分类，恢复它
		if existCategory.DeletedAt.Valid {
			...
			if err := global.DB(ctx.Request.Context()).Unscoped().Save(&existCategory).Error; err != nil {
	...
	// 创建新分类
	if err := global.DB(ctx.Request.Context()).Create(&category).Error; err != nil {
...
func (c *CategoryApi) UpdateCategory(ctx *gin.Context) {
	...
	var category model.Category
	if err := global.DB(ctx.Request.Context()).First(&category, req.ID).Error; err != nil {
	...
	if err := global.DB(ctx.Request.Context()).Model(&category).Updates(updateData).Error; err != nil {
...
func (c *CategoryApi) DeleteCategory(ctx *gin.Context) {
	...
	// 检查是否有图书使用此分类
	var count int64
	global.DB(ctx.Request.Context()).Model(&model.BookCategory{}).Where("category_id = ?", req.ID).Count(&count)
	if count > 0 {
	...
	if err := global.DB(ctx.Request.Context()).Unscoped().Delete(&model.Category{}, req.ID).Error; err != nil {
