...
| **Prometheus**| 抓取 /metrics 指标、存储时序数据           | 9090    |
| **Grafana**   | 可视化仪表盘、展示 QPS/延迟/Worker 等       | 3000    |
...
# 2. 启动完整栈（MySQL、Redis、migrate、api1/api2/api3、worker、scheduler、nginx、Prometheus、Grafana）
docker compose up -d
...
- `scheduler`：定时任务
- `nginx`：对外入口，端口 80
- `prometheus`：抓取 API 的 /metrics，端口 9090
...
| `METRICS_ENABLED`               | 是否启用 Prometheus 指标 | `true` / `false`                |
...
Metrics 用于暴露 Prometheus 格式的监控指标，供 Prometheus 抓取、Grafana 展示，用于监控服务健康、性能与异步链路状态。

**用途：**

- **HTTP 监控**：QPS、延迟分布、错误率（按 method、route、status 维度）
- **Worker 监控**：处理量、失败数、pending 积压
- **资源监控**：MySQL 连接池、Redis 延迟
...
| 指标名                                       | 类型        | 说明                                 |
| ----------------------------------------- | --------- | ---------------------------------- |
| `bookadmin_http_requests_total`           | Counter   | HTTP 请求总数，标签：method, route, status |
| `bookadmin_http_request_duration_seconds` | Histogram | HTTP 请求耗时分布，标签：method, route       |
| `bookadmin_worker_processed_total`        | Counter   | Worker 处理消息数，标签：stream, result     |
| `bookadmin_worker_failures_total`         | Counter   | Worker 失败次数，标签：stream, stage       |
| `bookadmin_worker_pending_messages`       | Gauge     | Redis Stream 待处理消息数，标签：stream      |
| `bookadmin_db_open_connections`           | Gauge     | 数据库连接池总连接数                         |
| `bookadmin_db_idle_connections`           | Gauge     | 空闲连接数                              |
| `bookadmin_db_in_use_connections`         | Gauge     | 使用中连接数                             |
| `bookadmin_redis_ping_duration_ms`        | Gauge     | Redis ping 延迟（毫秒）                  |


**访问方式：**

- **容器化部署**：Nginx 将 `/metrics` 代理到后端，访问 `http://localhost/metrics`（或实际域名）
- **本地开发**：直接访问 `http://localhost:8888/metrics`
- **验证**：`curl http://localhost/metrics` 可查看原始 Prometheus 文本格式

**配置：**

- `config.yaml` 中 `metrics.enabled` 控制是否注册 `/metrics` 路由，默认 `true`
- 环境变量 `METRICS_ENABLED` 可覆盖（如 `false` 关闭）

**Grafana 展示（已实现）：**

`docker compose up -d` 会启动 Prometheus 与 Grafana。Prometheus 自动抓取 api1/api2/api3 的 `/metrics`，Grafana 已预置 Prometheus 数据源与「Bookadmin 监控」仪表盘。

- **Grafana**：http://localhost:3000，默认账号 `admin` / `admin`
- **Prometheus**：http://localhost:9090，可查看 Targets 与原始指标
- **仪表盘内容**：HTTP QPS、P50/P99 延迟、MySQL 连接池、Redis 延迟、Worker 积压与处理速率
